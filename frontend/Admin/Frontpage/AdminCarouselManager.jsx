...
export default function AdminCarouselDashboard() {
    ...
    const fetchImages = async () => {
        try {
            const res = await axios.get("http://localhost:5000/api/carousel-images/all");
            setImages(res.data);
        } catch (err) {
    ...
    const handleUpload = async (e) => {
        ...
        try {
            setUploading(true);
            const res = await axios.post("http://localhost:5000/api/carousel-images", formData, {
    ...
    return (
        <div className="min-h-screen bg-gray-100 flex justify-center">
            <div className="w-full max-w-4xl bg-white shadow-lg p-4 rounded-lg mt-0">
                <h1 className="text-2xl font-bold text-center mb-6">🖼️ Admin Carousel Dashboard</h1>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                    {images.map(({ _id, url }) => (
                        <div key={_id} className="relative border rounded overflow-hidden shadow-sm">
                            <img src={url} alt="Carousel" className="w-full h-40 object-cover" />
                            <button
