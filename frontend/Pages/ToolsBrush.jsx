...
const ToolsBrushProducts = () => {
  const [products, setProducts] = useState([]);
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    const fetchToolsBrushProducts = async () => {
      setLoading(true);
      try {
        const res = await axios.get(
          `${import.meta.env.VITE_API_URL}/api/products/all/products`,
          {
            params: { category: "brushes"}, // 👈 Only fetch Tools & Brush products
          }
        );
  ...
  return (
    <div className="container mx-auto px-4 py-4 max-w-6xl">
      ...
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-sm sm:text-lg font-semibold text-gray-800 mb-1">Tools & Brush Products</h2>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
        {products.length ? (
          ...
          <p className="text-sm text-gray-500">
            {loading ? "Loading..." : "No Tools & Brush products available"}
          </p>
        )}
      </div>
    </div>
...
export default ToolsBrushProducts;
