...
export default function ProductDetails() {
  ...
  useEffect(() => {
    axios.get(`http://localhost:5000/api/products/details/${id}`)
  ...
  return (
    <div className="max-w-5xl mx-auto p-4 grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <div className="flex flex-col items-center">
        <img src={product.imageUrl} alt={product.name} className="w-full max-w-sm rounded-xl shadow-md" />
        <div className="flex gap-2 mt-2">
          {product.images?.map((img, i) => (
            <img key={i} src={img} alt="thumb" className="w-16 h-16 object-cover rounded cursor-pointer border" />
          ))}
        </div>
      </div>
      ...
      <div>
        ...
        <div className="flex items-center gap-3 mt-2">
          ...
          <span className="bg-green-100 text-green-700 text-sm px-2 py-1 rounded">In Stock</span>
        </div>
