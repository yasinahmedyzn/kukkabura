    ...
    const fetchHaircareProducts = async () => {
      ...
      try {
        const res = await axios.get(
          ...
          {
            params: { category: "hair" }, // 👈 Only fetch Hair Care products
          }
        );
  ...
  return (
    <div className="container mx-auto px-4 py-4 max-w-6xl">
      ...
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-sm sm:text-lg font-semibold text-gray-800 mb-1">Hair-care Products</h2>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
        {products.length ? (
          ...
          <p className="text-sm text-gray-500">
            {loading ? "Loading..." : "No Hair care products available"}
          </p>
        )}
      </div>
    </div>
